    ...
    public class ShipmentsController : WarehouseBaseController
    {
        ...
        public ActionResult Create(InboundReceiptFormViewModel model)
        {
            ...
            try
            {
                ...
                // Create receipt - Mặc định kho tổng (ID = 1)
                ...
                if (receipt.status == "completed")
                {
                    ...
                    {
                        // Log error but don't fail - allow user to complete manually
                        TempData["ErrorMessage"] = "Phiếu đã tạo nhưng lỗi cập nhật tồn kho: " + invEx.Message + ". Vui lòng hoàn thành thủ công.";
                ...
                TempData["SuccessMessage"] = "Tạo phiếu nhập kho thành công!" + 
                    (receipt.status == "completed" ? " Tồn kho đã được cập nhật." : "");
                return RedirectToAction("Details", new { id = receipt.id });
            }
            catch (Exception ex)
            {
        ...
        // POST: Warehouse/Shipments/Complete/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Complete(long id)
        {
            try
            {
                ...
                {
                    TempData["ErrorMessage"] = "Chỉ có thể hoàn thành phiếu ở trạng thái 'Chờ xử lý'.";
                ...
                // Update inventory
                UpdateInventory(id, receipt.warehouse_id);

                TempData["SuccessMessage"] = "Đã hoàn thành phiếu nhập kho! Tồn kho đã được cập nhật.";
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = "Lỗi khi hoàn thành: " + ex.Message;
            }
